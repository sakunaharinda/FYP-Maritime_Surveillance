...
class UCF24_3DHead(data.Dataset):
    num_classes = 24
    ...
    def __init__(self, opt, root, split, input_type='rgb', num_frames = 8):
        super(UCF24_3DHead, self).__init__()
        full_test = False
        self.data_dir = root
        ...
        self.coco_annots_path = os.path.join(root, 'splitfiles/coco_test_annots.json')
        self._imgpath = os.path.join(root, input_type)
        self._annopath = os.path.join(root, 'labels/', '%s.txt')
        trainlist, testlist, video_list, test_nframes = make_lists(root, input_type, split=1, fulltest=full_test)
        ...
        self.detector.model.ida_up._modules["node_2"]._modules["actf"]._modules.get("1").register_forward_hook(self.get_base_outputs)
    ...
    def get_base_outputs(self, model, input, output):
        self.outputs = output.detach().cpu().numpy()
