...
class CTDetDataset_UCF24(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    for i in range(start, frame_num + 1):
      img = cv2.imread(os.path.join(img_dir, '{:05d}.jpg'.format(i)))
      if flipped:
        img = img[:, ::-1, :]
    ...
    print (index, "pre-detector")
    self.detector.model(images)
    print (index, "post-detector")
    base_output = self.outputs
    print (index, "local variable")
    if base_output.shape[0] < self.num_frames:
      out_shape = base_output.shape
      padding = np.zeros((num_frames - out_shape[0], 
                          out_shape[1], out_shape[2], out_shape[3]))
      base_output = np.concatenate((padding, base_output), axis = 0)
    
    print (base_output.shape)
    out_shape = base_output.shape
    
    features = base_output.reshape(out_shape[0]*out_shape[1], out_shape[2], out_shape[3])
    ...
    ret = {'input': features, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh}
    if self.opt.dense_wh:
