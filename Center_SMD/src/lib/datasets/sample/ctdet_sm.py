...
class CTDetDataset_SM(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    im1 = cv2.filter2D(inp,-1,k1)
    im2 = cv2.filter2D(inp,-1,k2)
    im3 = cv2.filter2D(inp,-1,k3)
    im4 = cv2.filter2D(inp,-1,k4)
    im5 = cv2.filter2D(inp,-1,k5)

    res = np.concatenate((-prev_inp+im1,im2-prev_inp,im3-prev_inp,im4-prev_inp,im5-prev_inp),axis=0)
    #print("From ctdet_sm",res.shape)
    ...
    #ret = {'inp1': -prev_inp+im1,'inp2': -prev_inp+im2,'inp3': -prev_inp+im3,'inp4': -prev_inp+im4,'inp5': -prev_inp+im5, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'index':img_id, 'wh': wh}
    ret = {'input':res, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'index':img_id, 'wh': wh}
