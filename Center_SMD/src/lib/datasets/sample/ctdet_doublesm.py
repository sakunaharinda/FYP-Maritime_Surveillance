...
class CTDetDataset_DoubleSM(data.Dataset):
    def __init__(self):
      super(CTDetDataset_DoubleSM).__init__()
      self.kernels = {  0: np.array([[1,0,0], [0,0,0],[0,0,0]]),
    ...
    def _ssm_calc(self, var_img, ref_img, Gaussian = False, return_sim = False):
        ref_img = ref_img.transpose(1,2,0)
        var_img = var_img.transpose(1,2,0)
        return ssim(ref_img, var_img, multichannel=True, data_range = var_img.max()-var_img.min(), gaussian_weights = Gaussian, full = return_sim)
    ...
    def __getitem__(self, index):
        ...
        # concat_inp = np.concatenate((inp, prev_inp), axis = 0)
        ####### SMALL MOTION BEGINS #######
        im_motion = np.zeros((8, )+inp.shape, dtype=np.float32)
        struct_sim = []

        for i in self.kernels.keys():
            im_motion[i] = cv2.filter2D(inp, -1, self.kernels[i])
            struct_sim.append(self._ssm_calc(im_motion[i], prev_inp))

        struct_sim = np.array(struct_sim)
        ind_sorted = np.argsort(struct_sim)[::-1]       # argsorting in the descending order
        
        if self.split == 'train':
            rand_idx = random.sample(list(ind_sorted[:3]), k=1)

        else:
            rand_idx = ind_sorted[0]

        if self.opt.use_diff:
            cat_inp = inp + inp - prev_inp

        else:
            cat_inp = im_motion[rand_idx]
        
        if len(cat_inp.shape) > 3:
                cat_inp = cat_inp[0] 
  

        score, cat_inp = self._ssm_calc(cat_inp, prev_inp, Gaussian = self.opt.use_gaussian, return_sim=True)
        # score, cat_inp = self._ssm_calc(inp, prev_inp, Gaussian = self.opt.use_gaussian, return_sim=True)
        

        cat_inp = cat_inp.transpose(2,0,1)
        cat_inp = cat_inp.astype(np.float32)

        concat_inp = np.concatenate((inp, cat_inp), axis=0)
        ####### SMALL MOTION ENDS #######
